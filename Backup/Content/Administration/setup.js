...
//Dashboard chart
function setupDashboardChart(containerElementId,s1,s2,s3,s4,s5) {
...
function AgeChart(containerElementId, s1, s2, s3, s4,s5) {
   
   var ticks = ['5-12', '13-17', '18-24', '25-34', '35-44', '45-54', '55-64', '65+'];
   var plot1 = $.jqplot(containerElementId, [s1, s2, s3, s4,s5], {
        // The "seriesDefaults" option is an options object that will
        // be applied to all series in the chart.
        seriesDefaults: {
...
function drawPointsChart(containerElement,Imp,Det,Sh,Gro, days) {
    var cosPoints = [];
    var sinPoints = [];
    var powPoints1 = [];
    var powPoints2 = [];
    ...
    $.each(Imp, function (index, value) {
        cosPoints.push([index, value]); 
        });
    $.each(Det, function (index, value){ 
        sinPoints.push([index, value]); 
    });
    $.each(Sh, function (index, value) {
        powPoints1.push([index, value]); 
    });
    $.each(Gro, function (index, value) {
        powPoints2.push([index, value]); 
    });
