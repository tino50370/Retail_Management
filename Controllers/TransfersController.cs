...
namespace RetailKing.Controllers
{
    ...
    public class TransfersController : Controller
    {
        ...
        public ActionResult Index(string date, string enddate, string category, string ItemCode, string company, JQueryDataTableParamModel param)
        {
            ...
            var px = np.GetAllTransfers().Where(u => u.company == company ).ToList();
            px = (from emp in px
        ...
        [HttpPost]
        public ActionResult IndexSearches(string category, string ItemCode, string company, string SubCategory, JQueryDataTableParamModel param, string json_str, string Company)
        {
            NHibernateDataProvider np = new NHibernateDataProvider();
            var currentUserId = User.Identity.Name;
            char[] delimiter = new char[] { '~' };
        ...
        public ActionResult AuthorizeTransfers(string date, string enddate, string category, string ItemCode, string company, JQueryDataTableParamModel param)
        {
            NHibernateDataProvider np = new NHibernateDataProvider();
            //Inventory inv = new Inventory();
            //JQueryDataTableParamModel param = new JQueryDataTableParamModel();
        ...
        {
            NHibernateDataProvider np = new NHibernateDataProvider();
           var purch = db.Transfers.Where(u => u.Reciept.Trim() == Reciept).FirstOrDefault(); ;
...
//purch.StockedValue = purch.StockedValue + PLines.priceinc;
          //  if (purch.StockedValue == purch.total)
           ...
           var user = np.Getlogin(partz[1]);
            ...
            //np.SaveOrUpdateLossLines(purch);
                db.SaveChanges();
            return RedirectToAction("NewTransfers", new {LossId = purch.ID });
         
        }
        public ActionResult CompleteTransfer(string Reciept)
        {
            NHibernateDataProvider np = new NHibernateDataProvider();
            var purch = db.Transfers.Where(u => u.Reciept.Trim() == Reciept).FirstOrDefault(); ;
            // if (purch.StockedValue == null) purch.StockedValue = 0;
            ...
            //np.SaveOrUpdateLossLines(purch);
            db.SaveChanges();
            return RedirectToAction("Index", new { TransfersId = purch.ID });

        }
        #region Quotations
        public ActionResult NewTransfer(string Company, string TransfersId, JQueryDataTableParamModel param)
        {
            NHibernateDataProvider np = new NHibernateDataProvider();
            RetailKingEntities db = new RetailKingEntities();
           
            var xxx = ViewBag.Branch;
            TransfersQuote px = new Models.TransfersQuote();
            List<TransferingLine> lines = new List<TransferingLine>();
            var catz = (from e in np.GetAccountsByCode("3-") where e.AccountCode.Length == 4 select e).ToList();
            Account acc = new Account();
            acc.AccountName = "--Select A Category--";
            catz.Insert(0, acc);
          ...
          //  px.uzar
            ...
            if (string.IsNullOrEmpty(TransfersId))
            {
            ...
            {
                px.quote = db.Transfers.Find(long.Parse(TransfersId));
                if (px.quote != null)
                {
                    px.quotationlines = db.TransferingLines.Where(u => u.Reciept == px.quote.Reciept).ToList();
                }
                else
                {
        ...
        public ActionResult VoidGRVLineConfirmed(long id)
        {
            ...
            db.SaveChanges();
            return RedirectToAction("NewTransfers");
        ...
        public ActionResult newStock(string Qty, string ItemCode, string Company, string Reciept, string ItemName, string Supplier)
        {
            ...
            var purchz = db.PurchaseLines.Where(u => u.ItemCode.Trim() == ItemCode.Trim()).FirstOrDefault();
        ...
        public ActionResult newStock(string Qty, string ItemCode, string Company, string Reciept, string price, string item, string x)
        {
            if (item != "")
            {
                ...
                sl = db.Transfers.Where(u => u.Reciept == Reciept).FirstOrDefault() ;
                long id = sl.ID;
                ...
                foreach (var itm in parts)
                {
                    ...
                    TransferingLine  bs = new TransferingLine ();
                    bs.items = it.ItemName.Trim();
                    ...
                    np.SaveOrUpdateTransferingLines(bs);
                    ...
                    var pl = db.PurchaseLines.Where(u => u.ItemCode.Trim() == it.ItemCode.Trim() && u.Status == "OQ").FirstOrDefault();
                    if (pl != null)
                    {
                        if (pl.quantity >= slqty)
                        {
                            ...
                            TransferingLine  ss = new TransferingLine ();
                            ss.items = it.ItemName.Trim();
                        ...
                        else
                        {
                            ...
                            while (slqty > 0)
                            {
                                pl = db.PurchaseLines.Where(u => u.ItemCode.Trim() == it.ItemCode.Trim() && u.Status == "OQ").FirstOrDefault();
                                if (pl != null)
                                {
                                    if (pl.quantity >= slqty)
                                    {
                    ...
                    trn.ProcessSale("Transfers", decimal.Parse(par[2]), user.Location.Trim(), it.category, it.SubCategory, "");
                ...
                var sal = db.Transfers.Find(id);
                if (sal.total == null) sal.total = 0;
                    sal.total += amt;
                    sal.Reciept = user.prefix.Trim() + "-" + sl.ID;
                   // sal.Period = "0";
                    sal.State = "OQ";
                    sal.shift = user.username;//// SHIFT = USERNAME/CASHIER
                ...
                #endregion
                    return RedirectToAction("NewTransfer", new { Company = Company, TransfersId = sl.ID });
            }
        ...
        public ActionResult Purchases(string Company)
        {
            ...
            foreach (var nat in comp)
            {
                if (nat.name.Trim() != Company)
                {
                    yr.Add(new SelectListItem
                    {
                        Text = nat.name,
        ...
        [HttpPost]
        public ActionResult Purchases(Transfer quotation)
        {
            if (Request.IsAuthenticated)
            {
                ...
                db.Transfers.Add(quotation);
                db.SaveChanges();

                var uzar = np.Getlogin(partz[1]);
                // quotation.ID = id;
                quotation.company = uzar.Location;
                quotation.State = "OQ";
                ...
                return RedirectToAction("NewTransfer", new { Company = quotation.company,  TransfersId = quotation.ID });
